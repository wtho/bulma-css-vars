...
describe('fs helper', () => {
  ...
  test('checks if file starts with string', async () => {
    ...
    const existingFileDoesStartWith = await fileStartsWith(
      '/path/to/mock-file-1.sass',
      '@import "node_modules/'
    )
    const nonExistingFileDoesNotStart = await fileStartsWith(
      '/path/to/mock-file-2.sass',
      '@import"node_modules/'
    )
    const existingFileDoesNotStartWith = await fileStartsWith(
      '/path/to/mock-file-1.sass',
      'html {'
    )
